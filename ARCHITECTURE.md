...
- **Modular vanilla front-end, no bundler.** `index.html` loads focused ES modules directly: [`script.js`](lobby_worker/script.js) (the FSM driver) imports [`wasm.js`](lobby_worker/wasm.js) (WASM init + bindings), [`audio.js`](lobby_worker/audio.js) (Web Audio), [`overlays.js`](lobby_worker/overlays.js) (DOM overlays), and [`input.js`](lobby_worker/input.js) (keyboard/touch/mouse). There is intentionally no front-end build step.
- **Installable PWA with an offline app shell.** A service worker ([`sw.js`](lobby_worker/sw.js)) precaches the shell so the menu and the offline VS-AI game load without a network; [`pwa.js`](lobby_worker/pwa.js) registers it and shows an in-app "update available" prompt. The cache version is stamped per deploy at build time ([`scripts/stamp-sw.mjs`](scripts/stamp-sw.mjs)) — that byte change is what the browser picks up as an update. Dynamic routes (`/create`, `/ws/`, `/join/`) are never cached.
...
| **lobby_worker** | [`lobby_worker/`](lobby_worker/) | **The Lobby.** HTTP routing, match-code generation, and the static front-end (vanilla ES modules + a service-worker PWA). Re-exports `MatchDO`. | [`src/lib.rs`](lobby_worker/src/lib.rs) (router)<br>[`script.js`](lobby_worker/script.js) (FSM driver) + `wasm/audio/overlays/input/pwa.js` modules<br>[`sw.js`](lobby_worker/sw.js), `manifest.webmanifest`, `icons/` |
